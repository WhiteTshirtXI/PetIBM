"""
Plots the spatial convergence for the diffusive terms in 2D and in 3D.
"""
...
def plot_spatial_convergence(filepath, label, savepath):
  """
  Plots the relative errors in the diffusive terms versus the grid-spacing.
...
def main():
  ...
  plot_spatial_convergence(filepath, 'diffusive terms (2D)', savepath)
  # 3D cases
  filepath = os.path.join(os.getcwd(), 'data', 'relativeErrors3d.dat')
  savepath = os.path.join(os.getcwd(), 'images', 'convergence3d.png')
  plot_spatial_convergence(filepath, 'diffusive terms (3D)', savepath)
